...
print("\n" + "="*65)
print("STEP 1 - Downloading Pre-training Data")
print("="*65)
...
print(f"\n[3/4] OpenWebText (streaming {owt_target:,})...")
...
print(f"\n[4/4] TinyTextbooks (streaming {TEXTBOOKS_SAMPLES:,})...")
...
print("\n" + "="*65)
print("STEP 3 - Validating Dataset Formats")
print("="*65)

def show_samples(name, samples):
    print(f"\n{'─'*65}\n  {name}\n{'─'*65}")
    for si, (raw, u, a) in enumerate(samples):
        ok = "OK" if (u.strip() and a.strip()) else "EMPTY"
        print(f"\n  [Sample {si+1}]  {ok}")
...
print("\n" + "="*65)
print("STEP 5 - Streaming Tokenization -> Binary Files")
print("="*65)
...
with open(ft_path, "w", encoding="utf-8") as out_f:
    ...
    if openhermes_ds is not None:
        ...
        print(f"  Written OpenHermes: {oh_n:,}")
    ...
    if alpaca_ds is not None:
        ...
        print(f"  Written Alpaca: {alp_n:,}")

print(f"\n{'='*65}")
print("ALL DATA PREPARATION COMPLETE")
print(f"{'='*65}")
print(f"  data/tokenizer.json  vocab={actual_vocab:,}")
print(f"  data/train.bin       {n_train/1e9:.2f}B tokens")
print(f"  data/val.bin         {n_val/1e9:.3f}B tokens")
